...
export const ModelCombobox: React.FC<ModelComboboxProps> = ({
  ...
  return (
    <div className="combobox-root" ref={containerRef}>
      ...
      >
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1 truncate">
            {selectedModel ? (
              <span className="text-white text-xs font-medium truncate block font-mono">
      ...
      {/* Выпадающий список с поиском (сплошной непрозрачный фон) */}
      {isOpen && (
        <div className="combobox-dropdown">
          ...
          <div className="combobox-header">
            ...
            {provider === 'openrouter' && (
              <div className="combobox-pills custom-scrollbar">
                {[
                  { id: 'all', label: 'Все' },
                  ...
                  <button
                    ...
                    className={`combobox-pill ${selectedCategory === cat.id ? 'active' : ''}`}
                  >
                    {cat.label}
                  </button>
                ))}
          ...
          <div className="combobox-list custom-scrollbar">
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {isFree && (
                          <span className="px-1 py-0.1 text-[9px] font-bold bg-emerald-950 text-emerald-300 border border-emerald-700 rounded">
                            FREE
                          </span>
                        )}

                        {isReasoning && (
                          <span className="px-1 py-0.1 text-[9px] font-bold bg-indigo-950 text-indigo-300 border border-indigo-700 rounded flex items-center gap-0.5">
                            <Sparkles className="w-2 h-2" /> CoT
                          </span>
                        )}
                      </div>
            ...
            {searchQuery.trim() && !isExactMatch && (
              <button
                ...
                className="w-full text-left p-2 rounded-xl bg-indigo-950/80 hover:bg-indigo-900 border border-indigo-600 text-indigo-200 transition-colors flex items-center gap-2 text-xs"
              >
                <PlusCircle className="w-4 h-4 text-indigo-400 shrink-0" />
                <div className="truncate flex-1">
                  <span className="font-semibold block">Использовать кастомный ID:</span>
